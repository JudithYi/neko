...
contains
  ...
  subroutine user_initialize(t, u, v, w, p, coef, params)
    ...
    tstep = 0
    call python_setup(u%dof%Xh%lx, u%dof%Xh%ly, u%dof%Xh%lz, u%msh%nelv, u%msh%glb_nelv, u%msh%glb_nelv, &
            NEKO_COMM%MPI_VAL, iostep)
  ...
  subroutine user_write(tstep, iostep, u, v, w, p)
    ...
    call python_update(u%x, v%x, w%x, p%x, p%x)
